...
void handle_nat_outbound(struct sr_instance* sr,uint8_t * packet,unsigned int len,struct sr_if* rec_interface)
{
 ...
 if(protocol==ip_protocol_icmp)
 {
   ...
   struct sr_nat_connection *connection = sr_nat_lookup_conn(&(sr->nat),mapping,ipPacket->ip_dst,tcpPacket->dst_port);
   if (!connection) {
     if (tcp_hdr->ctrl == TCP_SYN) {
       connection = sr_nat_insert_conn(&(sr->nat), mapping, ipPacket->ip_dst, tcpPacket->dst_port);
     }
     else
       return;
     }
     ...
     ipPacket->ip_src = mapping->ip_ext;
     tcpPacket->sum = 0;
     tcpPacket->sum = tcpcksum(ipPacket, tcpPacket, len-sizeof(sr_ethernet_hdr_t)-sizeof(sr_ip_hdr_t));
     handle_ip(sr, packet, len);
   }
...
uint16_t tcpcksum(sr_ip_hdr_t *ipPacket, uint8_t *tcpPacket, unsigned int len) {
 ...
 /* Form frame  */
 unsigned int packet_len = sizeof(sr_tcp_pseudo_hdr_t) + len;
 uint8_t *packet = (uint8_t *)calloc(1, packet_len);

 sr_tcp_pseudo_hdr_t *tcp_pseudo_hdr = (sr_tcp_pseudo_hdr_t *)packet;

 tcp_pseudo_hdr->ip_src = ip_hdr->ip_src;
 tcp_pseudo_hdr->ip_dst = ip_hdr->ip_dst;
 tcp_pseudo_hdr->ip_p = ip_hdr->ip_p;
 tcp_pseudo_hdr->len = htons(len);

 /* Copy contents of tcp to this packet to calculate */
 memmove(packet + sizeof(sr_tcp_pseudo_hdr_t), tcp_msg, len);

 uint16_t tcp_sum = cksum(packet, packet_len);
 free(packet);
...
print_hdrs(packet,len);}
...
else
  {
    ...
    struct sr_arpreq *arp_request = sr_arpcache_queuereq(&sr->cache, next_hop->dest.s_addr, packet,len,next_interface->name);
